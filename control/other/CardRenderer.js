...
sap.ui.define([
    ...
    CardRenderer.render = function (oRm, oCard) {
        ...
        //start
        var eleFound = !!document.getElementById(oCard.getId()) ? document.getElementById(oCard.getId()).parentElement.getAttribute('class').search('grid-stack') : -1;


     /*   if (eleFound === -1) {
            oRm.write(`<div class="grid-stack-item"
                data-gs-width="${oCard.getProperty('cardWidth')}"
                data-gs-height="${oCard.getProperty('cardHeight')}">`); //data-gs-auto-position="true" data-gs-no-resize="false"
